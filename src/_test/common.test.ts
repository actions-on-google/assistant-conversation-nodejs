...
import { values, clone, stringify, toArray, sortJsonObject } from '../common'
...
test('sortJsonObject works with null values', t => {
  const unsorted = {
    c: '1',
    b: 2,
    a: null,
  }
  const sorted = sortJsonObject(unsorted)
  t.is(JSON.stringify(sorted), '{"a":null,"b":2,"c":"1"}')
})
